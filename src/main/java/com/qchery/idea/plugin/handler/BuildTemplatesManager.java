...
public class BuildTemplatesManager extends TemplatesManager {

    private static final String DEFAULT = "defaultBuilder.vm";

    public static BuildTemplatesManager getInstance() {
        return ServiceManager.getService(BuildTemplatesManager.class);
    }
    ...
    public TemplateResource[] getDefaultTemplates() {
        try {
            return new TemplateResource[]{
                    new TemplateResource("Fast Builder Default", readFile(DEFAULT), true),
            };
        } catch (IOException e) {
    ...
    protected static String readFile(String resource) throws IOException {
        return readFile(resource, BuildTemplatesManager.class);
    }
}
